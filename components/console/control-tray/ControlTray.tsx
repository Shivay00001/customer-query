...
function ControlTray({ children }: ControlTrayProps) {
  ...
  // Effect to update mic volume visualizer
  useEffect(() => {
    const onVolume = (volume: number) => {
      if (micButtonRef.current) {
        // The volume is a value between 0 and 1. We can scale it for a better visual effect.
        const scale = 50; // Adjust this scale factor as needed
        micButtonRef.current.style.setProperty('--volume', `${volume * scale}px`);
      }
    };
    ...
    return () => {
      ...
      if (micButtonRef.current) {
        // Reset the volume when not connected/muted
